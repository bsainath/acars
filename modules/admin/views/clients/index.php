...
<div class="box box-warning">
    ...
    border-top: 1px solid #ddd;">
        ...
        'method' => 'get',
			...
			<?php  $listData= ArrayHelper::map(TblAcaClients::findAll(['is_deleted'=>'0']), 'client_id', 'client_name');
               echo $form->field($searchModel, 'client_id')->dropDownList($listData, ['prompt'=>'Select','class'=>'form-control form-height'])->label(false);
 ?>
		<?php 	//	 $form->field($searchModel, 'client_name')->label(false); ?>
					<!-- <select class="form-control"  id="col1_filter" data-column="2">
							<option  value="">Select</option>
							
							<?php //if(!empty($recordsdropdown)) {
										
									//foreach($recordsdropdown as $key)
									//{
										?>
									<option value="<?php //echo $key['client_id']; ?>" ><?php //echo  $key['client_name']; ; ?></option>
									<?php //}} ?>
						</select>-->
	 ...
	 <?php ActiveForm::end(); ?>	
        ...
        'columns' => [
			...
			'value' => function ($model, $key, $index, $widget) {
				...
				return Html::a($clientno, ['/shadowlogin', 'id' =>$user_id], ['target'=>'_blank']);
			},
			'headerOptions' => [ 
					'style' => 'background-color:#428bca' 
			],
...
'buttons' => [
    'my_button' => function ($url, $model, $key) {
        ...
        return Html::a('<span class="fa fa-edit"></span>',['/admin/clients/editclient', 'id' => $user_id], ['class' => 'pointer']) ;
    },
]
			],
        ],
