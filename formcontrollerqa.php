...
class FormsController extends Controller {
	...
	public function actionIndex() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $get_company_id['c_id'] )) {
				...
				$model_acaformpricing = TblAcaFormPricing::find()->select('value')->where(['price_id'=>20])->one();
				$estimated_date ='+'.$model_acaformpricing->value.'d' ;
				$date = date('Y-m-d');
				$date_value =  date('Y-m-d', strtotime($date. ' + '.$model_acaformpricing->value.' days'));
					...
					if (! empty ( $model_1094c )) {
						...
						try {
							if ($post_values) {
								...
								$post_values ['part1']['name_of_ale_member_1_I']= $update_part1->name_of_ale_member_1_I;
								$post_values ['part1']['employer_identification_number_I']= $update_part1->employer_identification_number_I;
								
								$post_values ['part1']['street_address_1_I']= $post_values ['xml']['street_address_1__3'].' '.$post_values ['xml']['street_address_2__3'];
								$post_values ['part1']['street_Address_2_I']= $post_values ['xml']['street_address_1__11'].' '.$post_values ['xml']['street_address_2__11'];
								$post_values ['part1']['name_of_the_person_contact_I']= $post_values ['xml']['first_name__7'].' '.$post_values ['xml']['middle_name__7'].' '.$post_values ['xml']['last_name__7'].' '.$post_values ['xml']['suffix__7'];
								$post_values ['part1']['name_of_person_to_contact_2_I']= $post_values ['xml']['first_name__15'].' '.$post_values ['xml']['middle_name__15'].' '.$post_values ['xml']['last_name__15'].' '.$post_values ['xml']['suffix__15'];
								
								
								/************************************** part 1****************************************/
								if(!empty($post_values ['part1'])){		
								    $part_1 = serialize ( json_encode ( $post_values ['part1'] ) ); // serialising the array
								}
	...
	public function actionForm1095() {
		...
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			$previous_button = '';
			$next_button = '';
			$street_employer_address='';
			$street_employer_address2='';
				...
				if (! empty ( $check_company_details ) && in_array ( $client_id, $client_ids, TRUE ) && in_array ( $company_id, $mapped_company_ids, TRUE )) {
					...
					if (! empty ( $form_id ) && !empty($ssn)) {
						...
						if (! empty ( $model_1095c )) {
							...
							$post_values = \yii::$app->request->post ();
							$transaction = \Yii::$app->db->beginTransaction (); // begining the transaction
						//	print_r($update_part3->employer_self_insured__III);die();
							try {
							  ...
							  //  $post_values ['part1']['employee_city_or_town__I']=$post_values['part1']['employee_city_or_town__I'];
								...
								$post_values ['part2']['plan_start_month__II']= $update_part2->plan_start_month__II;
	...
	function actionCronset() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			try {
				...
				if ($model_cronset->save () && $model_cronset->validate ()) { // model save
					$transaction->commit ();
	...
	*/
			...
			try{
				...
				if(!empty ($post_values['TblAcaPrintAndMail'])){
					...
					if($values['person_type'] == 1){
						...
						if(!empty ($values['expedite_print'])){
					...
					if(!empty ($values['expedite_print'])){
					    ...
					    $model_printmail->expedite_print=$values['expedite_print'];
					}else{
						$model_printmail->expedite_processing_fee=0;
						$model_printmail->expedite_print=0;
					}
							...
							if(file_exists($filepath)){
								/***************details for mail**************************/
								 $model_print_mail = TblAcaPrintAndMail::find()->where(['print_id'=>$print_id])->one();
										...
										$from_email = 'adminmail@aca.com';
										$to_email = $model_print_mail->username->email;
	...
	private function Uploadpdffiles($filepath,$file_name,$person_type,$print_details)
	{
		...
		$result = array();
		  ...
		  $ftp_username = "SkyInsuranceTesting";
		  $ftp_userpass = "2bE6MfzM";
		  $ftp_server = "ftp1.carolina.sourcelink.com";
	...
	public function actionPrintandmaildetails(){
		...
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			$model_1095c_ssn = TblAca1095::find ()->select('ssn')->where(['form_id'=>$approved_form->id])->count();
				...
				$total_fees = ($count*$price_value)+ $package_cost ;
				$array_values = array (
						'name'=>$name,
						'value'=>$price_value,
						'count'=>$count,
	...
	private function actionCreate1094($company_id, $form_id, $created_by) {
		...
		try {
			...
			if (! empty ( $employe_details )) {
				...
				$model_lookupvalue = TblAcaLookupOptions::find()->select('lookup_value')->where(['lookup_id'=>$employe_details ['contact_person_suffix']])->one();
    ...
    //	$city = $dge_details->dge_city;
			...
			//	$part1 ['city_or_town_2_I'] = $dge_details ['dge_city'];
				...
				$model_lookupvalue='';
				$model_lookupvalue = TblAcaLookupOptions::find()->select('lookup_value')->where(['lookup_id'=>$dge_details ['dge_contact_suffix']])->one();
				if(!empty($model_lookupvalue)){
					$suffix =$model_lookupvalue->lookup_value;
				}else{
					$suffix ='';
		...
		*/
		 ...
		 */	
			...
			if ($plan_details ['total_aggregated_grp_months']) {
				
				$arr_individual_months_count = self::actionCountdetails ( $company_id, $plan_details ['total_aggregated_grp_months'] );
		...
		*/
			...
			// array_push($form_data,$part1);
			// array_push($form_data,$part2);
			// array_push($form_data,$part3);
			// array_push($form_data,$part4);
			
			// serialize(json_encode($form_data));
	...
	**/
	...
	private function actionGetresponsiblessn($company_id) {
		...
		try {
							...
							if ($periods->status == 1) {
								...
								if (! empty ( $medical_plan_class )) {
									if (empty ( $periods->termination_date ) || (date ( 'Y', strtotime ( $periods->termination_date ) ) == $reporting_year)) {
										...
										// function to get the eligibility_date TODO dont apply waiting period
			...
			// query to get the plan_type=ï¿½combinationï¿½ or plan_type=ï¿½self insuredï¿½.
...
private function actionGeteligibiltydate($waiting_period, $start_date, $value_3_1_1=0, $status=1) {
		...
		switch ($waiting_period) {
			...
			case 5 :
				$days = '';
				$temptermdate = '';
				$days = cal_days_in_month ( CAL_GREGORIAN, date ( 'm', strtotime ( $start_date ) ), date ( 'Y', strtotime ( $start_date ) ) );
				$temptermdate = date ( 'Y-m-' . $days, strtotime ( $start_date ) );
				$adddays=29+$value_3_1_1;
				$eligible_date = date ( 'Y-m-d', strtotime ( $temptermdate . "+$adddays day" ) );
				break;
			case 6 :
				$days = '';
				$temptermdate = '';
	...
	private function actionCreate1095forms($company_id, $form_id, $created_by) {
		...
		try {
			...
			if (! empty ( $result_unique_employees ['error'] )) {
				...
				// $unique_employees [0] = $result_unique_employees['success'][0];
				...
				if (! empty ( $result_unique_employees ['success'] [0] )) {
					...
					if (! empty ( $get_all_payroll_employee )) 					// start of # section (i)
					{
						foreach ( $get_all_payroll_employee as $employee ) {
							 ...
							 */
							$model_lookupvalue='';
							 $model_lookupvalue = TblAcaLookupOptions::find()->select('lookup_value')->where(['lookup_id'=>$employee->suffix])->one();
							if(!empty($model_lookupvalue)){
								$suffix =$model_lookupvalue->lookup_value;
							}else{
								$suffix ='';
								...
								foreach ( $get_employment_periods as $periods ) {
									...
									if (! empty ( $medical_plan_class )) {
										...
										if (strtotime ( $temptermdate ) < strtotime (date($reporting_year . '-01-01'))) {
											...
											// If (the record has checkbox as yes for â€œdonâ€™t apply waiting periodâ€�)
											...
											/*if ((strtotime ( $eligible_date ) <= strtotime ( $reporting_year . '-01-01' )) && (strtotime ( $temptermdate ) >= strtotime ( $reporting_year . '-12-31' ))) {
												$flag2 = 1;
											} else {
												$flag2 = 0;
											}*/
							...
							for($i = 1; $i <= 12; $i ++) {
								...
								if (! empty ( $arrcode [$employee->ssn] [$i] )) {
									...
									if (! empty ( $diff_codes )) {
										$arrcleancode [$employee->ssn] [$i] = array_values ( $arrcode [$employee->ssn] [$i] )[0];
									} else {
							...
							if (! empty ( $arrmonthvalue [$employee->ssn] )) {
								...
								if (count ( array_unique ( $arrmonthvalue [$employee->ssn] ) ) == 1) {
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_all_12_months__II'] = $arrmonthvalue [$employee->ssn] [1];
								} else {
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_jan__II'] = $arrmonthvalue [$employee->ssn] [1];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_feb__II'] = $arrmonthvalue [$employee->ssn] [2];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_march__II'] = $arrmonthvalue [$employee->ssn] [3];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_april__II'] = $arrmonthvalue [$employee->ssn] [4];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_may__II'] = $arrmonthvalue [$employee->ssn] [5];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_june__II'] = $arrmonthvalue [$employee->ssn] [6];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_july__II'] = $arrmonthvalue [$employee->ssn] [7];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_august__II'] = $arrmonthvalue [$employee->ssn] [8];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_september__II'] = $arrmonthvalue [$employee->ssn] [9];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_october__II'] = $arrmonthvalue [$employee->ssn] [10];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_november__II'] = $arrmonthvalue [$employee->ssn] [11];
									$arrformdata [$employee->ssn] ['2'] ['employee_required_contributions_december__II'] = $arrmonthvalue [$employee->ssn] [12];
								}
							}
												...
												if (empty ( $enrollment_periods->coverage_end_date ) || (date ( 'Y', strtotime ( $enrollment_periods->coverage_end_date ) ) == $reporting_year)) {
													...
													for($i = 1; $i <= 12; $i ++) {
														...
														if (empty ( $enrollment_periods->coverage_end_date )) {
 														...
 														//if (9.3 = â€œself Insuredâ€�) and (coverage_start_date<= start of the month) and (tempcoverage_end_Date>= end of the month) {
																...
																if(strtotime($eligible_date)<=strtotime($start_date_month)){
																	...
																	if(($periods->status == 2) || (strtotime($temptermdate)<strtotime($end_date_month))){
																		...
																		if( (!empty($enrollment_periods->coverage_start_date)) && (strtotime($enrollment_periods->coverage_start_date)<=strtotime($start_date_month)) && (strtotime($tempcoverageenddate)>=strtotime($end_date_month))){
																			...
																			//   if ( 9.3 ==Multi employer plan )
																			if ($medical_plan_class->plan_offer_type == 4) {
																				$arrcode16 [$employee->ssn] [$i] [] = '2E';
																			} elseif ($medical_plan_class_coverage_offered->tblAcaEmpContributions [0]->safe_harbor == 2) { // if (11.1 == ï¿½FTPï¿½ )
																				$arrcode16 [$employee->ssn] [$i] [] = '2G';
																			} elseif ($medical_plan_class_coverage_offered->tblAcaEmpContributions [0]->safe_harbor == 3) { // if (11.1 == ï¿½ROPï¿½ )
																				$arrcode16 [$employee->ssn] [$i] [] = '2H';
																			} elseif ($medical_plan_class_coverage_offered->tblAcaEmpContributions [0]->safe_harbor == 1) { // if (11.1 == ï¿½W2ï¿½ )
																				$arrcode16 [$employee->ssn] [$i] [] = '2F';
																			} elseif ($medical_plan_class_coverage_offered->tblAcaEmpContributions [0]->safe_harbor == 4) { // if (11.1 == ï¿½Not applicableï¿½ )
													...
													/*	// if (employementstart_date<= end of the month) and (temptermdate> = end of the month)
														if ((strtotime ( $periods->hire_date ) <= strtotime ( $end_date_month )) && (strtotime ( $temptermdate ) >= strtotime ( $end_date_month ))) {
															
															//    if  ( emp_status = FT) and (termination_date >= end of the month) {  
															if (($periods->status == 1) && (strtotime ( $periods->termination_date ) >= strtotime ( $end_date_month ))) {
																

																//if (eligib_date<= end of the month) and (eligb_date>=1st of the month)
																//if (eligib_date<= start of the month)   
																if (strtotime ( $eligible_date ) <= strtotime ($start_date_month)) {	


																
																	
																		// if ( coverage_start_date<= end of the month ) and ( coverage_enddate>= end of the month)
																		//if (strtotime ( $enrollment_periods->coverage_start_date ) <= strtotime ( $end_date_month ) && strtotime ( $enrollment_periods->coverage_end_date ) >= strtotime ( $end_date_month )) {
																		if (strtotime ( $enrollment_periods->coverage_start_date ) <= strtotime ( $end_date_month ) ) {	
																			$arrcode16 [$employee->ssn] [$i] [] = '2C';
																		} else {
							...
							//Checking for every month
							foreach($arrmonths as $key=>$value){

								$checkedcoverage[$value]=0;
								if(!empty($arrformdata [$employee->ssn] ['2'] ['section_4980h_safe_harbor_'.$value.'__II']) && $arrformdata [$employee->ssn] ['2'] ['section_4980h_safe_harbor_'.$value.'__II'] == '2C'){
							...
							//check  if any of the value in 16 line is 2C  and dependent employee is not present then the primary employee record should be there in 17 line
							if($valuetwoc > 0){							
												...
												if (($periods->ssn = $employee->ssn) && ($periods->person_type = '85' || $periods->person_type = '86')) {
													...
													if (empty ( $periods->coverage_end_date ) || (date ( 'Y', strtotime ( $periods->coverage_end_date ) ) == $reporting_year)) {
														...
														for($i = 1; $i <= 12; $i ++) {
															...
															if (strtotime ( $periods->coverage_start_date ) <= strtotime ( $start_date_month )) {
	...
	**/
	...
	private function GetmonthCode($month_flags) {
		...
		$code = '';
		// if (3.2 =ï¿½ï¿½) and ( 10.1.1 !=ï¿½ï¿½) and (10.2.1 !=ï¿½ï¿½) and (10.3 =yes) and (10.3.1 = yes) and (10.4 =yes)
		if (empty ( $month_flags ['3.2'] ) && ! empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 1) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 1) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 1)) {
			
			$code = '1k';
		} 	// if (3.2 !=ï¿½ï¿½ ) and ( 10.1.1 !=ï¿½ï¿½) and (10.2.1 !=ï¿½ï¿½) and (10.3 =yes) and (10.3.1 = no) and (10.4 =yes)
		elseif (! empty ( $month_flags ['3.2'] ) && ! empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 1) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 2) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 1)) {
		...
		elseif (empty ( $month_flags ['3.2'] ) && empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 1) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 1) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 1)) {
			...
			$code = '1F';
		} 		// if (3.2 = ï¿½ï¿½) and ( 10.1.1!=ï¿½ï¿½) and (10.2.1!=ï¿½ï¿½) and (10.3 =no) and (10.3.1 = no) and (10.4 =yes)
		elseif (empty ( $month_flags ['3.2'] ) && ! empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 2) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 2) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 1)) {
			
			$code = '1C';
		} 		// if (3.2 = ï¿½ï¿½) and ( 10.1.1!=ï¿½ï¿½) and (10.2.1!=ï¿½ï¿½) and (10.3 =no) and (10.3.1 = no) and (10.4 =no)
		elseif (empty ( $month_flags ['3.2'] ) && ! empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 2) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 2) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 2)) {
			
			$code = '1B';
		} 		// if (3.2 = ï¿½ï¿½) and ( 10.1.1!=ï¿½ï¿½) and (10.2.1!=ï¿½ï¿½) and (10.3 =yes) and (10.3.1 = yes) and (10.4 =no)
		elseif (empty ( $month_flags ['3.2'] ) && ! empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 1) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 1) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 2)) {
			
			$code = '1J';
		} 		// if (3.2 = ï¿½ï¿½) and ( 10.1.1!=ï¿½ï¿½) and (10.2.1!=ï¿½ï¿½) and (10.3 =yes) and (10.3.1 = no) and (10.4 =no)
		elseif (empty ( $month_flags ['3.2'] ) && ! empty ( $month_flags ['10.1.1'] ) && ! empty ( $month_flags ['10.2.1'] ) && (! empty ( $month_flags ['10.3'] ) && $month_flags ['10.3'] == 1) && (! empty ( $month_flags ['10.3.1'] ) && $month_flags ['10.3.1'] == 2) && (! empty ( $month_flags ['10.4'] ) && $month_flags ['10.4'] == 2)) {
		...
		if (empty ( $check_cron )) {
			...
			if (! empty ( $forms )) {
				foreach ( $forms as $form ) {
					...
					/*
					if (! file_exists ( 'files/temp_pdf' )) {
						mkdir ( 'files/temp_pdf/', 0777, true );
					}
					*/
					...
					/*
					if (! file_exists ( 'files/pdf/' . $encrypt_company_id )) {
						mkdir ( 'files/pdf/' . $encrypt_company_id, 0777, true );
					}
					*/
					...
					if (! file_exists ( 'files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id )) {
						mkdir ( 'files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id, 0777, true );
					}
					*/
					...
					if (! empty ( $arr_1094_pdf_data )) {
						...
						if (! empty ( $arr_1095_pdf_data )) {
							foreach ( $arr_1095_pdf_data as $each_pdf_form ) {
								...
								$part1 = array ();
								$each_form = array ();
								$part1 ['employer_name__I']='';
								$part1 ['employee_name__I']='';
								$part1 ['employee_street_Address__I']='';
								$part1 ['employer_street_address__I']='';
								$part1 ['employer_city_town__I']='';
								$part1 ['employee_city_or_town__I']='';
								$part1 ['employer_state_province__I']='';
								$part1 ['employer_country_and_zip__I']='';
								$part1 ['employee_city_or_town__I']='';
								$part1 ['employee_state_province__I']='';
								$part1 ['employee_country_and_zip_code__I']='';
								...
								// $file_1095 = 'files/temp_pdf/' . $company_id . '_' . $form_id . '_1095_data_filled.pdf';
								...
								//	$pdf = new Pdf ( $filepath_1095 );
								//	$pdf->fillForm ( $arr_full_ssn_data )->saveAs ( $file_1095 );
									
									$pdf = new Pdf ( $filepath_1095 );
									$pdf->fillForm ( $arr_masked_ssn_data )->compress()->flatten()->saveAs ( $masked_ssn_file_1095 );
								} else {
								//	$pdf = new Pdf ( $single_page_filepath_1095 );
								//	$pdf->fillForm ( $arr_full_ssn_data )->saveAs ( $file_1095 );
								...
								$pdf = new Pdf ( $address_1095_filepath );
								$pdf->fillForm ( [ 
								...
								] )->saveAs ( $file_address_name );
								
								$pdf = new Pdf ( [ 
										$file_address_name,
										$masked_ssn_file_1095 
								] );
							...
							/*	$pdf = new Pdf ( [ 
								...
								$pdf->compress ()->flatten ()->saveAs ( $file_address_name );
							*/	
								// deleting the file
							...
							for($j=1,$k=1; $j <= $i; $j++,$k++) {
								...
								if($k==50){
									...
									$pdf = new Pdf ( $arr_1095_merge_pdfs );
							...
							//print_r($arr_1095_masked_pdfs); die();
					//	$pdf = new Pdf ( $arr_1095_pdfs );
						//	$pdf->compress ()->flatten ()->saveAs ( $save_file_1095 );
							
						//	$pdf = new Pdf ( $arr_1095_masked_pdfs );
						//	$pdf->flatten ()->saveAs ( $save_masked_file_1095 );
							//->send();
							
							$arr_complete_merge_data=array_merge ( $arr_1095_partial_merged_pdfs,$arr_1095_merge_pdfs );
							
							$pdf = new Pdf ( $arr_complete_merge_data );
							...
							if($model_aca_pdf_forms->validate() && $model_aca_pdf_forms->save()){
								...
								if(!empty($check_print))
								{
									...
									if(file_exists($filepath)){
										...
										$from_email = 'adminmail@aca.com';
										$to_email = $check_print->username->email;
									...
									$from = 'adminmail@aca.com';
									$to = $forms->username->email;
									\Yii::$app->CustomMail->GeneratepdfMail ( $to, $from, $form_details ); 
